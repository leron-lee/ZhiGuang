        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data == "false") {
                    ...
                    $("#favDiv").show();
                    $("#favDiv").html("<div><span>加入收藏夹成功！</span><a href='" + url + "' style='color:red;'>查看收藏夹中的商品</a></div><div><br/><input type='button' value=' 确 定 ' onclick=\"$('#favDiv').hide()\"></div>");
                }
            },
            error: function() {
                //alert("cuowo");
    ...
    $.post("/API/CommonHandler.ashx?op=ShoppingCar", {}, function(d) {
        ...
        if (!!data) {
            $.each(data, function(i, content) {
                ...
                var item = $("<dl> <dt> <a href=/product_detail-" + content.ProductId + ".htm><img src='" + content.ThumbnailUrl60 + "' style='width:40px;height:40px'/></a> <em>×" + content.Quantity + "</em> </dt> <dd> <strong><em>￥</em>" + content.MemberPrice + "</strong><p><a href='#' onclick='DeleteShoppingCart(\"" + content.SkuId + "\")'>删除</a></p> </dd> </dl>");
                $(".cartInner").append(item);
            });
            $(".cartList h2").hide();
            $(".cartInner").show();
            $(".cartInner").append($("<p class='checkOrder'><a href='/ShoppingCart.aspx'>查看我的购物车</a></p>"));
        } else {
            $(".cartList h2").show();
            $(".cartInner").hide();
        }
...
function LoginAndBuy2() {
    ...
    var loginhtml = "您好，{0}<a href='/user/UserDefault.aspx'>会员中心</a> <b><a onclick='return todoout()' href='/logout.aspx'>退出</a></b>";
    $.ajax({
...
function supisapply() {
    ...
    $.ajax({
        ...
        case 1:
            location.href = "/ApplicationSucc.html";
            flog = true;
            break;
        case 2:
...
function todoout() {
    var flog = false;
    $.post("/loginout.ashx").done(function() {
            location.href = location.href;
            flog = false;
        })
        .fail(function() {
