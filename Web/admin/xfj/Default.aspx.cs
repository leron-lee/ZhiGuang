...
namespace Web.admin.xfj
{
    public partial class Default : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            if (!base.IsPostBack)
            {
                if (base.Request.QueryString["del"] != null)
                {
                    ...
                    new SqlHelper().ExecuteNonQuery("delete from xfj_jilu where id = " + del);
        ...
        public void fy()
        {
            ...
            else
            {
                string sqlabc = string.Concat(new object[]
				{
					"select top ",
					(p - 1) * x,
					" id from xfj_jilu where id <> 0 ",
					sqlw,
					" order by id desc"
				});
                string nid = new access().ExecuteScalar("select top 1 id from xfj_jilu where id in (" + sqlabc + ") order by id");
                sqlstring = string.Concat(new object[]
				{
					"select top ",
					x,
					" * from xfj_jilu where id <> 0 and id < ",
					nid,
					" ",
					sqlw,
					" order by id desc"
            ...
            this.fystr = en_page.fystr_one(x, "xfj_jilu where id <> 0 " + sqlw, url, p, "pagex", "id", 1);
        }
        protected void cao_bt_Click(object sender, System.EventArgs e)
        {
            ...
            if (sid != "")
            {
                get.xfj_cao(username, cao, jf, bz);
